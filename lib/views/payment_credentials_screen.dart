...
class PaymentCredentialsScreen extends StatelessWidget {
  ...
  const PaymentCredentialsScreen({super.key, required this.arguments});
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(
      ...
      tag: 'credentials-${arguments.orderId}-${arguments.paymentMethodId}',
    );
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            arguments.isCashOnDelivery
                                ? 'confirm_order'.tr
                                : 'payment_title'.trParams({
                                    'method': arguments.paymentMethod,
                                  }),
                            textAlign: TextAlign.center,
                          ...
                          const SizedBox(height: 28),
                          _AmountCard(
                            formattedAmount: controller.formattedAmount,
                          ),
                          const SizedBox(height: 26),
                          _PaymentInformationCard(arguments: arguments),
                          const SizedBox(height: 30),
                          SizedBox(
                            height: 58,
                            child: FilledButton(
  ...
  String _buttonLabel() {
    if (arguments.isCashOnDelivery) {
      return 'place_order'.tr;
    }

    if (arguments.paymentMethodId == 'khalti') {
      return 'continue_to_gateway'.trParams({'gateway': 'KHALTI'});
    }

    if (arguments.paymentMethodId == 'esewa') {
      return 'continue_to_gateway'.trParams({'gateway': 'ESEWA'});
    }

    return 'continue_to_payment'.tr;
  }
}

class _PaymentLogo extends StatelessWidget {
  const _PaymentLogo({required this.arguments, required this.isCompact});
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        decoration: BoxDecoration(
          shape: BoxShape.circle,
          color: arguments.paymentColor.withValues(alpha: 0.12),
        ),
        child: Image.asset(
          arguments.paymentAsset,
          fit: BoxFit.contain,
...
class _AmountCard extends StatelessWidget {
  ...
  const _AmountCard({required this.formattedAmount});
...
class _PaymentInformationCard extends StatelessWidget {
  const _PaymentInformationCard({required this.arguments});
  ...
  Widget build(BuildContext context) {
    ...
    final description = arguments.isCashOnDelivery
        ? 'cod_pay_delivery'.tr
        : 'secure_checkout_description'.trParams({
            'method': arguments.paymentMethod,
          });
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, color: arguments.paymentColor),
          ),
...
class _ProcessingContent extends StatelessWidget {
  const _ProcessingContent({required this.message});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Flexible(
          child: Text(
            message.isEmpty ? 'please_wait'.tr : message,
